...
# Step 5: Install Poetry for the second repository
log_message "STEP 5: Installing Poetry for the second repository"
log_message "✓ Installing pipx (if not already installed)"
pip install --user pipx 2>&1 | tee -a "$LOG_FILE"
pipx ensurepath 2>&1 | tee -a "$LOG_FILE"

log_message "✓ Installing/upgrading Poetry using pipx"
pipx install --upgrade poetry 2>&1 | tee -a "$LOG_FILE"

if [ $? -eq 0 ]; then
    log_message "✓ Poetry installed/upgraded successfully"
else
    log_message "ℹ Trying alternative installation method with pip"
    pip install --user --upgrade poetry 2>&1 | tee -a "$LOG_FILE"
    
    if [ $? -eq 0 ]; then
        log_message "✓ Poetry installed/upgraded successfully using pip"
    else
        log_message "✗ ERROR: Failed to install/upgrade Poetry"
        exit 1
    fi
fi

# Step 6: Install dependencies with Poetry for the second repository
log_message "STEP 6: Installing dependencies with Poetry for the second repository"
