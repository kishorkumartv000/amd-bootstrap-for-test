...
log_message "Control flags: INSTALL_DEPS_REPO1=$INSTALL_DEPS_REPO1, INSTALL_DEPS_REPO2=$INSTALL_DEPS_REPO2, INSTALL_POETRY=$INSTALL_POETRY"
...
# Step 3: Install dependencies from the first repository (if enabled)
if [ "$INSTALL_DEPS_REPO1" = true ]; then
    log_message "STEP 3: Installing dependencies from the first repository"
    cd "$TARGET_DIR1" || {
    ...
    log_message "SKIPPING STEP 3: Dependency installation for first repository is disabled"
fi

# Step 4: Install/upgrade tidal-dl-ng from the second repository (if enabled)
if [ "$INSTALL_DEPS_REPO2" = true ]; then
    ...
    log_message "SKIPPING STEP 4: Dependency installation for second repository is disabled"
fi

# Step 5: Install pipx properly and add to PATH (if Poetry installation is enabled)
if [ "$INSTALL_POETRY" = true ]; then
    ...
    log_message "SKIPPING STEP 5: pipx installation is disabled (Poetry installation disabled)"
fi

# Step 6: Install Poetry using pipx (if enabled)
if [ "$INSTALL_POETRY" = true ]; then
    ...
    log_message "SKIPPING STEP 6: Poetry installation is disabled"
fi

# Step 7: Install dependencies with Poetry for the second repository (if enabled)
if [ "$INSTALL_DEPS_REPO2" = true ] && [ "$INSTALL_POETRY" = true ]; then
    ...
    log_message "SKIPPING STEP 7: Poetry dependency installation is disabled"
fi
