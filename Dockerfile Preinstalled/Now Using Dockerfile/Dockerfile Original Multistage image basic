...
# 1. Install build dependencies (with ffmpeg for building & testing)
...
# ✅ Full tidal-dl-ng config with ffmpeg path
RUN mkdir -p /root/.config/tidal_dl_ng && \
    cat > /root/.config/tidal_dl_ng/settings.json <<'EOF'
{
    "skip_existing": true,
    "lyrics_embed": false,
    "lyrics_file": false,
...
# Runtime dependencies (⚡ ffmpeg installed explicitly here)
...
COPY --from=builder /root/.config/tidal_dl_ng/ /root/.config/tidal_dl_ng/
